...
### スマホ・タブレット（タッチ操作）
タッチ端末では画面に**バーチャルパッド**が自動表示されます。
- 左下の**スティック**で移動
- 右下のボタンで **跳**（ジャンプ）/ **攻撃** / **必殺** / **盾**（シールド）
- 上部の **START ▶** で決定（スタート・キャラ決定・リトライ）
- 横向き（ランドスケープ）推奨

### スマホですぐ遊ぶ（GitHub Pages）
リポジトリの **Settings → Pages → Source: Deploy from a branch** で
ブランチ `claude/smash-game-prototype-m2zdj` ・フォルダ `/(root)` を選んで Save。
数十秒後に `https://massinalkaloid.github.io/game/` で遊べます（スマホのブラウザでURLを開くだけ）。
